...
static void turn_backlight(struct backlight_device *backlight, int status)
{
	if( !backlight )
		return ;
...
static int ili9881c_disable(struct drm_panel *panel)
{
	...
	turn_backlight(tftcp->backlight, 0);
...
static int ili9881c_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	if (backlight) {
		tftcp->backlight = of_find_backlight_by_node(backlight);
