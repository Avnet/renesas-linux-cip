    ...
    switch( isp_func )
    {
        case ISP_FUNC_ALL:
            ...
            if( (all_set_bit & ISP_SET_ALLDATA_3DNR) != 0 ){
                unsigned char nr[16];
                nr[0]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+3];   //Y_THRES_A(L)
                nr[1]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+4];   //Y_THRES_A(H)
                nr[2]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+5];   //Y_THRES_B(L)
                nr[3]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+6];   //Y_THRES_B(H)
                nr[4]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+7];   //Y_TILT(L)
                nr[5]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+8];   //Y_TILT(H)
                nr[6]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+10];  //C_THRES_A(L)
                nr[7]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+11];  //C_THRES_A(H)
                nr[8]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+12];  //C_THRES_B(L)
                nr[9]   = all_data[ISP_SET_ALLDATA_POS_3DNR_D+13];  //C_THRES_B(H)
                nr[10]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D+14];  //C_TILT(L)
                nr[11]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D+15];  //C_TILT(H)
                nr[12]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D];     //Y COEF
                nr[13]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D+1];   //C COEF
                nr[14]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D+2];   //Y ALPHA MAX
                nr[15]  = all_data[ISP_SET_ALLDATA_POS_3DNR_D+9];   //C ALPHA MAX
                pmem_write( isp_work_adr + ISP_PARAM_OFFSET_3DNRD, &nr[0], 16);// 3DNR Parameter area(16byte)
                int_val_3dnr = -1;
                /* Save data using ISP_set function */
