...
static t_isp_wakeup_entry ispdone_entry_head;;
...
void isp_ctrl_init(struct file *file)
{
	...
	drpLevel = ISP_CTRL_DRP_LEVEL_DEF_VAL;

	INIT_LIST_HEAD(&ispdone_entry_head.list);
}
...
void isp_ctrl_ispdone_add_list_tail(int error, unsigned int index)
{
    t_isp_wakeup_entry* set_entry;
	...
	set_entry->errCode = error;
	set_entry->index = index;
	list_add_tail(&set_entry->list, &ispdone_entry_head.list);
	return;
}
...
t_isp_wakeup_entry* isp_ctrl_ispdone_get_list(void)
{
	t_isp_wakeup_entry *entry = NULL;

	if (list_empty(&ispdone_entry_head.list)) {
	}else{
		entry = list_entry(ispdone_entry_head.list.next, t_isp_wakeup_entry, list);
	}
...
void isp_ctrl_ispdone_del_list(void)
{
	t_isp_wakeup_entry* del_ptr;

	del_ptr = list_entry(ispdone_entry_head.list.next, t_isp_wakeup_entry, list);
    list_del(ispdone_entry_head.list.next);
    kfree(del_ptr);
	return;
}
...
static void isp_ctrl_ispdone_clear_list(void)
{
	t_isp_wakeup_entry *entry;
	t_isp_wakeup_entry *entryNull = NULL;
	...
	if (list_empty(&ispdone_entry_head.list)) {
		...
		list_for_each_entry_safe(entry,entryNull,&ispdone_entry_head.list,list){
			list_del(&entry->list);
			kfree(entry);
		}
	}
...
static int isp_ctrl_isp_start(struct file *file)
{
	...
	if( ret < 0 ){
		isp_ctrl_dprintk("[error] %s: return ISP_open, ret=%d\n", __func__, ret);
		return ret;
	}
	isp_ctrl_dprintk("[info] %s: return ISP_open, ret=%d\n", __func__, ret);
...
static int isp_ctrl_isp_finish(void)
{
	...
	isp_ctrl_wakeup_show_list();
#endif
	isp_ctrl_dprintk("[info] %s: call ISP_close ret = %d\n", __func__, ret);
	return ret;
}
...
void isp_ctrl_buffer_done_main(void)
{
	...
	if(ret<0)
	{
		isp_ctrl_dprintk("[err]%s ISP_go err %d\n",__func__,ret);
		waitCallback = 0;
...
//	unsigned char expose_p;
	...
	while(!kthread_should_stop())
	{
        ...
        isp_ctrl_dprintk("[info]%s isp_callback waiting...\n",__func__);
        add_wait_queue(&cru_wait_queue,&wait);
		schedule();
		remove_wait_queue(&cru_wait_queue,&wait);
	    isp_ctrl_dprintk("[info]%s isp_callback called\n",__func__);
    	...
    	if(setAE.ae_on)
    	{
	    	...
	    	isp_ctrl_dprintk("[info]%s ISP_AE_Get gain:%d  exposure=%d\n",__func__,gain_p,expose_p);
	    	//[I2C] set gain
			if(ispOpenInfo.gain)
	    	{
				isp_ctrl_dprintk("[info]%s ov5645_set gain=%d\n",__func__,gain_p);
				v4l2_ctrl_s_ctrl(ispOpenInfo.gain,gain_p);
			}
			//[I2C] set shutter speed
			if(ispOpenInfo.expose)
			{
				isp_ctrl_dprintk("[info]%s ov5645_set expose=%d\n",__func__,expose_p);
...
static void isp_ctrl_isp_callback(int result,unsigned long out_adr)
{
	...
	spin_lock_irqsave(&wakeup_list_lock, flags);
	if( result<0 ){
		isp_ctrl_ispdone_add_list_tail(result, err_dqbuf_index);
...
static int isp_ctrl_set_format(struct file *file)
{
	...
	if ( ret ){
		...
		isp_ctrl_dprintk("[info] %s: success setting csi2 format.\n", __func__);
		isp_ctrl_dprintk("[info] %s: changeFmt.format.width = %d, changeFmt.format.height = %d, changeFmt.format.field = %d\n", __func__, changeFmt.format.width, changeFmt.format.height, changeFmt.format.field);
	}
	...
	if ( ret ){
		...
		isp_ctrl_dprintk("[info] %s: success setting ov5645 format.\n", __func__);
		isp_ctrl_dprintk("[info] %s: changeFmt.format.width = %d, changeFmt.format.height = %d, changeFmt.format.field = %d\n", __func__, changeFmt.format.width, changeFmt.format.height, changeFmt.format.field);
	}
	return ret;
}
...
{
	isp_ctrl_dprintk("[info] %s: start. value = %d\n", __func__, *(input));
    return func(file, fh, input);
}
...
{
	int ret;
	isp_ctrl_dprintk("[info] %s: start, opend=%d\n", __func__, ispOpenInfo.opened);
...
int isp_ctrl_dqbuf(ioctl_dqbuf func, struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	if( getEntry != NULL ){
		...
		if( getEntry->index == buf->index ){
			...
			if( ( 0 != retErrCode ) && ( -EBUSY != retErrCode ) )
			{
				retErrCode = -EINVAL;
			}
	...
	if( ispdonf_chkf != 0 )
	{
		getEntry = isp_ctrl_ispdone_get_list();
		if( getEntry != NULL ){
			ret = getEntry->errCode;
			if( ( 0 != ret ) && ( -EBUSY !=ret ) )
			{
				ret = -EINVAL;
			}
			...
			spin_lock_irqsave(&wakeup_list_lock, flags);
			isp_ctrl_ispdone_del_list();
			spin_unlock_irqrestore(&wakeup_list_lock, flags);
		}
	}
	isp_ctrl_dprintk("[info] %s: finish ret = %d \n", __func__,ret);
	dqbufprintk("d0 finish r=%d",ret);
	return ret;
}

int isp_ctrl_chk_index_complete( int index )
{
	int ret = 0;
	t_isp_wakeup_entry* getEntry;
	getEntry = isp_ctrl_wakeup_get_list();
	if( getEntry != NULL ){
		if (getEntry->index == index)
		{
			ret = 1;
		}
	}
...
{
	int ret = 0;
	isp_ctrl_dprintk("[info] %s: V4L2_CID_RZ_ISP_IN_FMT val=%d\n", __func__, value);
	// check param
...
{
	int ret = 0;
	isp_ctrl_dprintk("[info] %s: V4L2_CID_RZ_ISP_FRM_INTERVAL val=%d\n", __func__, value);
	// check param
	if( value <  0 ){
		ret = -EINVAL;
	}else{
...
{
	int ret = 0;
	isp_ctrl_dprintk("[info] %s: V4L2_CID_RZ_ISP_DRP_LV val=%d\n", __func__, value);
	if( value < 0 ){
		ret = -EINVAL;
	}else{
		drpLevel = value;
...
static int isp_ctrl_set_ISP_AE_Set(int value, int* dstValue)
{
	...
	isp_ctrl_dprintk("[info] %s: org  ae_on = %d, expose_level = %d, target_y = %d, threshold = %d\n",
			__func__, setAE.ae_on, setAE.expose_level, setAE.target_y, setAE.threshold);
	oldVal = *dstValue;
	*dstValue = value;
	isp_ctrl_dprintk("[info] %s: set  ae_on = %d, expose_level = %d, target_y = %d, threshold = %d\n",
			__func__, setAE.ae_on, setAE.expose_level, setAE.target_y, setAE.threshold);
	ret = ISP_AE_Set(setAE.ae_on, setAE.expose_level, setAE.target_y, setAE.threshold);
	if( ret < 0 ){
		// undo
		*dstValue = oldVal;
		isp_ctrl_dprintk("[info] %s: undo ae_on = %d, expose_level = %d, target_y = %d, threshold = %d\n",
				__func__, setAE.ae_on, setAE.expose_level, setAE.target_y, setAE.threshold);
	}
	isp_ctrl_dprintk("[info] %s: fix ae_on = %d, expose_level = %d, target_y = %d, threshold = %d\n",
			__func__, setAE.ae_on, setAE.expose_level, setAE.target_y, setAE.threshold);
	isp_ctrl_dprintk("[info] %s: ret=%d\n", __func__, ret);
	return ret;
}
...
int isp_ctrl_s_ctrl(ioctl_s_ctrl func, struct v4l2_ctrl *ctrl)
{
	...
	switch(ctrl->id)
	{
		...
		case V4L2_CID_RZ_ISP_AWB:
			isp_ctrl_dprintk("[info] %s: V4L2_CID_RZ_ISP_AWB\n", __func__);
	...
	isp_ctrl_dprintk("[info] %s: finish\n", __func__);
	return ret;
}
...
int isp_ctrl_g_ext_ctrl(struct v4l2_ext_control *ctrl)
{
	...
	isp_ctrl_dprintk("[info] %s: start\n", __func__);
	isp_ctrl_dprintk("[info] %s: ctrl = 0x%p, id = %d\n", __func__, ctrl, ctrl->id);
	...
	if( i >= ISP_CTRL_G_EXT_CTRLS_NUM_ID ){
		...
		{
			case V4L2_CID_RZ_ISP_IN_FMT:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_IN_FMT route\n", __func__);
				ctrl->value = ispOpenInfo.inFMT;
				break;
			case V4L2_CID_RZ_ISP_FRM_INTERVAL:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_FRM_INTERVAL route\n", __func__);
				ctrl->value = ispOpenInfo.frameInterval;
				break;
			case V4L2_CID_RZ_ISP_DRP_LV:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_DRP_LV route\n", __func__);
				ctrl->value = drpLevel;
				break;
			case V4L2_CID_RZ_ISP_AE:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_AE route\n", __func__);
				ISP_get_param(NULL, NULL, NULL, &ctrl->value, NULL, NULL, NULL);
				break;
			case V4L2_CID_RZ_ISP_EXPOSE_LV:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_EXPOSE_LV route\n", __func__);
				ISP_get_param(NULL, NULL, NULL, NULL, &ctrl->value, NULL, NULL);
				break;
			case V4L2_CID_RZ_ISP_T_BL:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_T_BL route\n", __func__);
				ISP_get_param(NULL, NULL, NULL, NULL, NULL, &ctrl->value, NULL);
				break;
			case V4L2_CID_RZ_ISP_THRESHOLD:
				isp_ctrl_dprintk("%s: V4L2_CID_RZ_ISP_THRESHOLD route\n", __func__);
				ISP_get_param(NULL, NULL, NULL, NULL, NULL, NULL, &ctrl->value);
				break;
			default:
				// do not pass
				isp_ctrl_dprintk("[info] %s: default route\n", __func__);
				ret = -EINVAL;
				break;
		}
	}else{
		switch(ctrl->id)
		{
			...
			default:
				// do not pass
				isp_ctrl_dprintk("[info] %s: ISP_get default route\n", __func__);
				ret = -EINVAL;
				break;
		}
	}

	isp_ctrl_dprintk("[info] %s: finish, ret = %d, value = %d\n", __func__, ret, ctrl->value);
	return ret;
}
